...
|---|---|---|---|---|
| FDM | Lax–Friedrichs + van Leer TVD, dx = 1/64, CFL-RK4, 164 steps | **9.49e-4** overall (train 9.53e-4, test 9.34e-4; median 7.68e-4, max 5.12e-3; mean relative L∞ 1.12e-3) | ~45 ms | 1× |
| Spectral (reference) | Fourier pseudo-spectral + IF-RK4, N = 2048, dt = 1e-3 | ~1e-6 relative to the Cole–Hopf exact solution | ~3800 ms | — |
| FNO | L = 4, k_max = 16, d_v = 64, GELU, Adam | **~8.6e-3** | ~0.5 ms | ~90× (≈7600× vs. spectral) |
...
establishes **9.49e-4 at N = 64** as the accuracy level the FNO must beat on this benchmark.
...
1. **Reliable reference data.** GRF initial conditions (tau = 7, alpha = 2, K_KL = 128) with spectral reference
   ...
   all three viscosities (7.6e-9 … 5.3e-6). The FDM baseline shows clean second-order grid convergence.
2. **Large efficiency gain.** On the nu = 0.01, N = 64 benchmark the FNO is ~90× faster than the FDM baseline and
   ~7600× faster than the spectral solver, with an error about 9× larger than the FDM error (8.6e-3 vs. 9.49e-4).
